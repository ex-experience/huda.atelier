(()=>{
  ...
  function renderCart(){
    ...
    box.innerHTML=cart.length?cart.map(x=>'<div class="cart-item"><div>'+esc(x.brand)+'<br><strong>'+esc(x.name)+'</strong><br>'+x.qty+' × '+x.price+' SAR</div><button data-remove="'+x.id+'">×</button></div>').join(''):'<p class="note">سلتك فارغة بعد.</p>';
  ...
  q('[data-checkout]')?.addEventListener('click',async ()=>{
    ...
    const guidance='خصم 10٪ على Event Signature أو Skin Architecture خلال 45 يوماً بعد اكتمال الطلب. العرس والسفر مستثنيان. الدرجة تُؤكد قبل الشحن.';
  ...
  const KB=[
    ...
    {k:['خصم','امتياز','privilege'],a:'بعد طلب مسبق مؤهل مكتمل: خصم 10٪ على Event Signature أو Skin Architecture خلال 45 يوماً. العرس الكامل والسفر خارج هذه النافذة.'},
